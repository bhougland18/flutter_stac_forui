...
prepare_flutter_sdk() {
  ...
  # Reset bin/cache to be a real directory
  if [ -e "$flutter_root/bin/cache" ]; then
    rm -rf "$flutter_root/bin/cache"
  fi
  mkdir -p "$flutter_root/bin/cache"

  # Link everything from nix store's bin/cache
  for item in "$nix_flutter_root/bin/cache"/*; do
    name=$(basename "$item")
    ln -s "$item" "$flutter_root/bin/cache/$name"
  done

  # Overwrite the linux-x64/flutter_tester with our patched version
  # We have to remove the symlink first
  ...
  local target_tester="$flutter_root/bin/cache/artifacts/engine/linux-x64/flutter_tester"
  if [ -e "$target_tester" ]; then
    echo "== Overwriting flutter_tester with patched version =="
    rm -f "$target_tester"
    cp ".cache/flutter-tester-patched" "$target_tester"
    chmod +x "$target_tester"
  fi
